...
export const BASE_PATH = "https://platform.barndoor.ai".replace(/\/+$/, "");
...
/**
 * The OAuth 2.0 endpoints this API authenticates against, from the spec's
 * `OAuth2` security scheme. Generated — do not edit.
 *
 * `OAUTH_ISSUER` is what an OIDC client needs for discovery; the other two are
 * the flow endpoints discovery would resolve to.
 *
 * Scoped to the authorizationCode flow because `authMethods` carries one entry
 * PER FLOW — emitting from every OAuth entry would declare each constant twice.
 * That flow is the only one holding all three values.
 *
 * OAUTH_ISSUER is omitted entirely when the spec declares no `x-issuer`, rather
 * than emitted empty: a missing issuer then fails to compile where it is used,
 * instead of reaching an OIDC discovery call as "".
 */

export const OAUTH_ISSUER = "https://auth.barndoor.ai/realms/barndoor";
export const OAUTH_AUTHORIZATION_URL = "https://auth.barndoor.ai/realms/barndoor/protocol/openid-connect/auth";
export const OAUTH_TOKEN_URL = "https://auth.barndoor.ai/realms/barndoor/protocol/openid-connect/token";
